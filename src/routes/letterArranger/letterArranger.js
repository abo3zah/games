...
export const LetterArranger = () => {
  ...
  const [checks, setChecks] = useState(Array.from({ length: 3 }, () => false))
  ...
  const updatePuzzle = () => {
    ...
    for (let i = 0; i < word.length; i++) {
      tempLetters.push(<LetterPuzzelComponent key={i} checks={checks[i]} value={i} letter={word[i]} />);
    }
  ...
  useEffect(() => {
    setPause(false)
    updatePuzzle()
    setChecks(Array.from({ length: levels[selectedItem].word.length }, () => false))
  }, [selectedItem])
  ...
  return (
    <div className='w-full p-6 grid justify-items-center content-center gap-3 select-none'>
      <div className='p-6 grid justify-items-center align-middle md:w-1/3 w-full bg-gray-50 gap-3 rounded outline outline-black'>
        ...
        <img src={levels[selectedItem].image} className="w-1/2 h-50" alt={levels[selectedItem].word}></img>
        <div className={`flex flex-wrap gap-3 justify-center text-5xl`}>
          {letters}
        </div>
        <hr className='border border-black w-full' />
        <Keyboard pause={pause} selectedItem={selectedItem} levels={levels} changeCheck={changeCheck} />
      </div>
